...
export default function SiteHero() {
  ...
  useEffect(() => {
    ...
    const photo = textCol.querySelector<HTMLElement>("[data-hero-photo]")
    ...
    if (photo) { photo.style.opacity = "0"; photo.style.transform = "translateY(16px)" }
    ...
    if (photo) tl.add(photo, { opacity: 1, translateY: 0, duration: 500 }, "-=200")
  ...
  return (
    <section className="light-section border-b border-border">
      <div
        ref={innerRef}
        className="min-h-[calc(100vh-4rem)] pt-24 md:pt-28 pb-16 md:pb-20 max-w-5xl mx-auto px-6 flex items-center relative overflow-hidden"
      >
        {/* Logos flottants arrière-plan */}
        <div data-float data-float-delay="0" data-float-dur="6000" data-float-range="18"
          aria-hidden className="pointer-events-none absolute top-16 right-0 md:right-10 opacity-[0.07] select-none">
          <Image src="/meta-logo.png" alt="" width={140} height={140} className="object-contain md:w-[200px] md:h-[200px]" />
        </div>

        <div data-float data-float-delay="800" data-float-dur="7500" data-float-range="12"
          aria-hidden className="pointer-events-none absolute bottom-20 right-2 md:right-16 opacity-[0.06] select-none text-[#D97706]"
          style={{ width: 90, height: 90 }}>
          <FacebookIcon className="w-full h-full" />
        </div>

        <div data-float data-float-delay="1600" data-float-dur="5200" data-float-range="10"
          aria-hidden className="pointer-events-none absolute bottom-32 left-0 opacity-[0.05] select-none">
          <Image src="/meta-logo.png" alt="" width={70} height={70} className="object-contain" />
        </div>

        <div data-float data-float-delay="2400" data-float-dur="8200" data-float-range="20"
          aria-hidden className="pointer-events-none absolute top-10 left-[-20px] md:left-[-40px] opacity-[0.04] select-none text-[#D97706]"
          style={{ width: 120, height: 120 }}>
          <FacebookIcon className="w-full h-full" />
        </div>

        {/* Contenu */}
        <div className="grid md:grid-cols-[1fr_420px] gap-10 md:gap-14 items-center w-full relative z-10">

          <div ref={textColRef} className="flex flex-col gap-5 md:gap-6">
            <p data-hero-label className="font-mono text-xs text-primary tracking-widest uppercase">
              Expert Facebook Ads · Cotonou, Bénin
            </p>

            <h1 data-hero-h1 className="font-display text-[36px] sm:text-[48px] md:text-[60px] leading-[1.1] tracking-tight text-foreground font-bold">
              5 ans à aider les entreprises à vendre plus.
            </h1>

            <p data-hero-p className="text-base md:text-lg text-muted-foreground max-w-[480px] leading-relaxed">
            ...
            <div data-hero-photo className="md:hidden flex justify-center py-2">
              <div className="relative">
                ...
                <div className="relative w-44 h-44 rounded-full overflow-hidden border-2 border-primary/50">
                  <Image
            ...
            <div data-hero-buttons className="flex flex-col sm:flex-row gap-3 pt-1">
              <Link
                ...
                className={cn(
                  ...
                  "h-12 px-6 md:px-8 text-sm md:text-base font-semibold w-full sm:w-auto justify-center"
                )}
              >
              ...
              <Link
                ...
                className={cn(
                  ...
                  "h-12 px-6 md:px-8 text-sm md:text-base w-full sm:w-auto justify-center"
                )}
              >
                Voir les services
              </Link>
          ...
          {/* Photo circulaire — desktop uniquement */}
